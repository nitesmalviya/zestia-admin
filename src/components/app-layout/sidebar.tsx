...
import { logoutAction } from '@/utils/graphql/auth/logout-action';

const Sidebar = ({ sidebarOpen, setSidebarOpen }: Readonly<{ sidebarOpen: boolean; setSidebarOpen: (open: boolean) => void }>) => {
  ...
  return (
    ...
    >
      ...
      >
        <Image src="/img.png" alt="" fill priority sizes="250px" style={{ objectFit: 'cover', objectPosition: 'center bottom' }} />
      </div>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: sidebarOpen ? '12px 12px' : '12px 0', justifyContent: sidebarOpen ? 'flex-start' : 'center' }}>
          <Image src="/admin-avatar.png" alt="Avatar" width={34} height={34} style={{ borderRadius: '50%' }} />
          {sidebarOpen ? (
            <div style={{ flex: 1 }}>
